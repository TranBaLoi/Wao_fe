...
//    private lateinit var bottomNavigationView: BottomNavigationView
    ...
    private var healthConnectTotalCaloriesBurned: Double? = null
    ...
    private fun fetchDashboardData() {
        lifecycleScope.launch {
            ...
            if (summaryResult is ApiResult.Success) {
                ...
                // Keep backend state in sync and let Health Connect override the fields it owns.
            ...
            when (HealthConnectManager.getSdkStatus(this@MainActivity)) {
                ...
                HealthConnectClient.SDK_AVAILABLE -> {
                    ...
                    if (grantedPermissions.containsAll(HealthConnectManager.readPermissions)) {
                        ...
                        startHealthSnapshotLoop(healthConnectClient)
                        if (initiatedByUser) {
                            Toast.makeText(
                                this@MainActivity,
    ...
    private fun loadHealthConnectMetrics(healthConnectClient: HealthConnectClient) {
        lifecycleScope.launch {
            refreshHealthConnectSnapshot(healthConnectClient)
        }
    }

    private suspend fun refreshHealthConnectSnapshot(healthConnectClient: HealthConnectClient) {
        runCatching {
            ...
            healthConnectTotalCaloriesBurned = snapshot.totalCaloriesBurnedToday
            ...
            Log.d(
                "MainActivity",
                "Health snapshot steps=${snapshot.stepsToday}, activeCalories=${snapshot.activeCaloriesBurnedToday}, totalCalories=${snapshot.totalCaloriesBurnedToday}, heartRate=${snapshot.latestHeartRateBpm}",
            )
            renderHealthConnectMetrics()
        }.onFailure { error ->
            ...
            healthConnectTotalCaloriesBurned = null
    ...
    private fun startHealthSnapshotLoop(healthConnectClient: HealthConnectClient) {
    ...
    private fun stopHealthSnapshotLoop() {
        healthSnapshotJob?.cancel()
        healthSnapshotJob = null
    }
    ...
    private fun renderCaloriesSummary() {
        val caloriesOut = healthConnectTotalCaloriesBurned
        ...
        if (netCalories > targetCaloriesGoal) {
            tvCalRemainingLabel.text = "Vượt quá"
            tvCalRemaining.text = (netCalories - targetCaloriesGoal).toInt().toString()
            pbCalories.progressTintList = android.content.res.ColorStateList.valueOf(android.graphics.Color.RED)
        } else {
            tvCalRemainingLabel.text = "Còn lại"
            tvCalRemaining.text = remaining.toInt().coerceAtLeast(0).toString()
    ...
    private fun showHealthConnectPermissionDialog() {
        AlertDialog.Builder(this)
            ...
            .setMessage(
                "Wao needs read access to your step count, calories burned, and heart rate so we can connect the app to Health Connect.",
            )
    ...
    private fun showHealthConnectInstallDialog() {
        AlertDialog.Builder(this)
            ...
            .setMessage(
                "Health Connect is required before Wao can ask for step count, calories burned, and heart rate permissions.",
            )
            .setNegativeButton("Later", null)
